 ...
 *         WITsHOUT ANY WARRANTY; without even the implied warranty of
...
if (is_array($cron)) 
{
	...
	if (($i_now - $i_next) >= 0) 
	{
		require_once 'osc/cron.hourly.php';
...
if (is_array($cron)) 
{
	...
	if (($i_now - $i_next) >= 0) 
	{
		require_once 'osc/cron.daily.php';
		// update the next execution time in t_cron
...
if (is_array($cron)) 
{
	...
	if (($i_now - $i_next) >= 0) 
	{
		require_once 'osc/cron.weekly.php';
		// update the next execution time in t_cron
