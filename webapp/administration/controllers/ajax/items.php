...
class CAdminAjax extends AdminSecBaseModel
{
	...
	function doModel() 
	{
		...
		case 'items': // Return items (use external file administration/ajax/item_processing.php)
			require_once osc_admin_base_path() . '/controllers/ajax/items_processing.php';
		...
		case 'media': // Return items (use external file administration/ajax/media_processing.php)
			require_once osc_admin_base_path() . '/controllers/ajax/media_processing.php';
		...
		case 'upgrade': // AT THIS POINT WE KNOW IF THERE'S AN UPDATE OR NOT
					...
					{ // Everything is OK, continue
						...
						if ($handle = opendir(ABS_PATH . 'oc-temp')) 
						{
							...
							{ // Everything is OK, continue
								 ...
								 ************************/
								$error_queries = array();
								if (file_exists(osc_lib_path() . 'osc/installer/struct.sql')) 
								{
									$sql = file_get_contents(osc_lib_path() . 'osc/installer/struct.sql');
								...
								{ // Everything is OK, continue
									...
									if ($rm_errors == 0) 
									{
										$message = __('Everything was OK! Your OpenSourceClassifieds installation is updated');
									}
									else
									{
										$message = __('Almost everything was OK! Your OpenSourceClassifieds installation is updated, but there were some errors removing temporary files. Please, remove manually the "oc-temp" folder');
			...
			if ($remove_error_msg != '') 
			{
				if ($error == 0) 
				{
					$message.= "<br /><br />" . __('We had some errors removing files, those are not super-sensitive errors, so we continued upgrading your installation. Please remove the following files (you already have OpenSourceClassifieds upgraded, but to ensure maximun performance)');
				}
			}
			if ($error == 5) 
			{
