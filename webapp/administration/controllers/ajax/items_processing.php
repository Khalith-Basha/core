	...
	private function toDatatablesFormat() 
	{
		...
		if (count($this->result) > 0) 
		{
			...
			foreach ($this->result as $aRow) 
			{
				...
				$addr = array();
				if ($aRow['s_address'] != '' && $aRow['s_address'] != null) 
				{
					$addr[] = $aRow['s_address'];
				};
				if ($aRow['s_city'] != '' && $aRow['s_city'] != null) 
				{
					$addr[] = $aRow['s_city'];
				};
				if ($aRow['s_zip'] != '' && $aRow['s_zip'] != null) 
				{
					$addr[] = $aRow['s_zip'];
				};
				if ($aRow['s_region'] != '' && $aRow['s_region'] != null) 
				{
					$addr[] = $aRow['s_region'];
				};
				if ($aRow['s_country'] != '' && $aRow['s_country'] != null) 
				{
					$addr[] = $aRow['s_country'];
				};
