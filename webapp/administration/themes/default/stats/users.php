...
osc_show_flash_message('admin'); ?>
                ...
                <div>
                    <div style="padding: 20px;">
                        <p>
                            <a href="<?php
echo osc_admin_base_url(true); ?>?page=stats&action=users&type_stat=day"><?php
_e('Last 10 days'); ?></a>
                            <a href="<?php
echo osc_admin_base_url(true); ?>?page=stats&action=users&type_stat=week"><?php
_e('Last 10 weeks'); ?></a>
                            <a href="<?php
echo osc_admin_base_url(true); ?>?page=stats&action=users&type_stat=month"><?php
_e('Last 10 months'); ?></a>
                        </p>
                    </div>
                </div>
...
?>
	...
	foreach ($users_by_country as $k => $v) 
	{
		...
		echo "data_country.setValue(" . $k . ", 1, " . $v['num'] . ");";
                ...
                new google.visualization.PieChart(document.getElementById('by_country')).draw(data_country, {title:"<?php
	_e('Users per country'); ?>"});
	...
	foreach ($users_by_region as $k => $v) 
	{
		...
		echo "data_region.setValue(" . $k . ", 1, " . $v['num'] . ");";
                ...
                // Create and draw the visualization.
                new google.visualization.PieChart(document.getElementById('by_region')).draw(data_region, {title:"<?php
	_e('Users per region'); ?>"});
            }
            </script>
        <?php
