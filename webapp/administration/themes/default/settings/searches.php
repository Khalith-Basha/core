...
_e('Save latest searches'); ?></label>
                                    <br/>
                                    <label><?php
_e('OpenSourceClassifieds could save the latest searches users do on your site. You could have an idea of what your users are looking for or show them on the site as "hot topics".'); ?></label>
                                </fieldset>
                            </div>
                            ...
                            <div style="float: left; width: 50%;">
                                <fieldset>
                                    ...
                                    <div style="font-size: small; margin: 0px;">
                                        ...
                                        <?php
if (osc_purge_latest_searches() != 'hour' && osc_purge_latest_searches() != 'day' && osc_purge_latest_searches() != 'week' && osc_purge_latest_searches() != 'forever' && osc_purge_latest_searches() != '1000') 
{
	$custom_checked = true;
}
else
...
_e('Number of files to keep'); ?>:</label> <input type="text" <?php
echo (($custom_checked) ? 'value="' . osc_purge_latest_searches() . '"' : ''); ?> onkeyup="javascript:document.getElementById('customPurge').value = this.value;"/>
                                        <input type="hidden" name="customPurge" id="customPurge" value="<?php
echo osc_purge_latest_searches(); ?>" />
                                        <br/>
                                        <label><?php
_e('Latest searches functionality could generate a lot of data. It\'s recomended you purge that data. You could delete one hour old, one day old or one week old, you could also specify the number of searches to keep or to never delete them.'); ?></label>
                                    </div>
                                </fieldset>
                            </div>
