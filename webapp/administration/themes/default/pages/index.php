...
osc_current_admin_theme_path('header.php'); ?>
        ...
        <script type="text/javascript">
            function order_up(id) {
                ...
                $.ajax({
                    url: "<?php
echo osc_admin_base_url(true) ?>?page=ajax&action=order_pages&id="+id+"&order=up",
            ...
            function order_down(id) {
                ...
                $.ajax({
                    url: "<?php
echo osc_admin_base_url(true) ?>?page=ajax&action=order_pages&id="+id+"&order=down",
                    success: function(res){
                        oTable.fnClearTable();
                        json = eval( '(' + res + ')') ;
                        oTable.fnAddData(json);
...
?>
		...
		echo osc_static_page_id(); ?>' />",
                                    "<?php
		echo $page['s_internal_name']; ?><div id='datatables_quick_edit'>" +
                                    "<a href='<?php
		...
		echo osc_static_page_id(); ?>);' style='cursor:pointer;width:15;height:15px;' src='<?php
		...
		echo $last_id != osc_static_page_id() ? ',' : ''; ?>
                        <?php
	};
}; ?>
                              ],
...
echo osc_current_admin_theme_url('images/pages-icon.png'); ?>" alt="" title=""/></div>
            ...
            <script type="text/javascript">
                $(document).ready(function() {
                    ...
                    $('#up').live('mouseover', function(event) {
                        $(this).attr('src', '<?php
echo osc_current_admin_theme_url('images/arrow_up_dark.png'); ?>');
                    });
