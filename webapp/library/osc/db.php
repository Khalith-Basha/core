...
class DB
{
	...
	function osc_dbExec() 
	{
		...
		default:
			...
			foreach ($argv as & $arg) $arg = $this->db->real_escape_string($arg);
			unset($arg);
	...
	function osc_dbFetchValues() 
	{
		...
		{
			$this->debug($sql);
			while ($result = $qry->fetch_array()) $results[] = $result[0];
			$qry->free();
		}
		else
		{
	...
	function osc_dbFetchResults() 
	{
		...
		{
			$this->debug($sql);
			while ($result = $qry->fetch_assoc()) $results[] = $result;
			$qry->free();
		}
		else
		{
		...
		foreach ($tables as $v) 
		{
			...
			if (array_key_exists(strtolower($table), $struct_queries)) 
			{
				...
				if (preg_match('|\((.*)\)|ms', $struct_queries[strtolower($table) ], $match)) 
				{
					...
					{
						if (strtolower(substr(trim($index), 0, 2)) != 'on') 
						{ // && strtolower(substr(trim($index),0,7))!='foreign') {
