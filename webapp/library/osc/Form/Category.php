...
{
	public function primary_input_hidden($category) 
	{
		parent::generic_input_hidden("id", $category["pk_i_id"]);
	}
	public function category_select($categories, $category, $default_item = null, $name = "sCategory") 
	{
		...
		foreach ($categories as $c) 
		{
			...
			{
				$this->subcategory_select($c['categories'], $category, $default_item, 1);
			}
		}
		echo '</select>';
	}
	public function subcategory_select($categories, $category, $default_item = null, $deep = 0) 
	{
		...
		foreach ($categories as $c) 
		{
			...
			if (isset($c['categories']) && is_array($c['categories'])) 
			{
				$this->subcategory_select($c['categories'], $category, $default_item, $deep);
			}
		}
	}
	public function categories_tree($categories = null, $selected = null, $depth = 0) 
	{
		if ($categories != null && is_array($categories)) 
		{
			...
			foreach ($categories as $c) 
			{
				...
				$this->categories_tree($c['categories'], $selected, $depth + 1);
				echo '</li>';
			}
			echo '</ul>';
		}
