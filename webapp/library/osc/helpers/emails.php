...
function fn_email_alert_validation($alert, $email, $secret) 
{
	...
	$params = array('subject' => $_title, 'to' => $email, 'to_name' => $user['s_name'], 'body' => $body, 'alt_body' => $body);
	osc_sendMail($params);
}
...
function fn_email_comment_validated($aComment) 
{
	...
	{
		$words = array();
		$words[] = array('{COMMENT_AUTHOR}', '{COMMENT_EMAIL}', '{COMMENT_TITLE}', '{COMMENT_BODY}', '{WEB_URL}', '{ITEM_URL}', '{ITEM_LINK}');
		$words[] = array($aComment['s_author_name'], $aComment['s_author_email'], $aComment['s_title'], $aComment['s_body'], osc_base_url(), osc_item_url(), '<a href="' . osc_item_url() . '">' . osc_item_url() . '</a>');
		...
		$emailParams = array('subject' => $title, 'to' => $aComment['s_author_email'], 'to_name' => $aComment['s_author_name'], 'body' => $body, 'alt_body' => $body);
...
function fn_email_new_item_non_register_user($item) 
{
	...
	$words[] = array($item['pk_i_id'], $item['s_contact_name'], $item['s_contact_email'], osc_base_url(), $item['s_title'], $item_url, osc_page_title(), '<a href="' . $edit_url . '">' . $edit_url . '</a>', $edit_url, '<a href="' . $delete_url . '">' . $delete_url . '</a>', $delete_url);
...
function fn_email_user_forgot_password( $user, $password_url ) 
{
	...
	if( !is_null($content) )
	{
		...
		$words[] = array($user['s_name'], $user['s_email'], '<a href="' . osc_base_url() . '" >' . osc_base_url() . '</a>', osc_page_title(), $_SERVER['REMOTE_ADDR'], '<a href="' . $password_url . '">' . $password_url . '</a>', $password_url, date('Y-m-d H:i:') . '00');
...
function fn_email_send_friend($aItem) 
{
	...
	$params = array('add_bcc' => $add_bcc, 'from' => $aItem['yourEmail'], 'from_name' => $aItem['yourName'], 'subject' => $title, 'to' => $aItem['friendEmail'], 'to_name' => $aItem['friendName'], 'body' => $body, 'alt_body' => $body);
	osc_sendMail($params);
}
...
function fn_email_item_inquiry($aItem) 
{
	...
	$emailParams = array('add_bcc' => $add_bcc, 'from' => $from, 'from_name' => $from_name, 'subject' => $title, 'to' => $item['s_contact_email'], 'to_name' => $item['s_contact_name'], 'body' => $body, 'alt_body' => $body, 'reply_to' => $yourEmail);
...
function fn_email_item_validation($item) 
{
	...
	$words[] = array($all, $item['s_description'], $item['s_country'], osc_prepare_price($item['i_price']), $item['s_region'], $item['s_city'], $item['pk_i_id'], $item['s_contact_name'], $item['s_contact_email'], '<a href="' . osc_base_url() . '" >' . osc_base_url() . '</a>', $item['s_title'], $item_url, osc_page_title(), '<a href="' . $validation_url . '" >' . $validation_url . '</a>', $validation_url);
...
function fn_email_admin_new_item($item) 
{
	...
	$words[] = array('<a href="' . $admin_edit_url . '" >' . $admin_edit_url . '</a>', $admin_edit_url, $all, $item['s_description'], $item['s_country'], osc_prepare_price($item['i_price']), $item['s_region'], $item['s_city'], $item['pk_i_id'], $item['s_contact_name'], $item['s_contact_email'], '<a href="' . osc_base_url() . '" >' . osc_base_url() . '</a>', $item['s_title'], $item_url, osc_page_title(), '<a href="' . $validation_url . '" >' . $validation_url . '</a>', $validation_url);
...
function fn_email_item_validation_non_register_user($item) 
{
	...
	$words[] = array($all, $item['s_description'], $item['s_country'], osc_prepare_price($item['i_price']), $item['s_region'], $item['s_city'], $item['pk_i_id'], $item['s_contact_name'], $item['s_contact_email'], '<a href="' . osc_base_url() . '" >' . osc_base_url() . '</a>', $item['s_title'], $item_url, osc_page_title(), '<a href="' . $validation_url . '" >' . $validation_url . '</a>', $validation_url, '<a href="' . $edit_url . '">' . $edit_url . '</a>', $edit_url, '<a href="' . $delete_url . '">' . $delete_url . '</a>', $delete_url);
...
function fn_email_admin_new_user( array $user )
{
	...
	if (!is_null($content)) 
	{
		...
		$title = osc_mailBeauty(osc_apply_filter('email_title', osc_apply_filter('email_user_registration_title', $content['s_title'])), $words);
		$body = osc_mailBeauty(osc_apply_filter('email_description', osc_apply_filter('email_user_regsitration_description', $content['s_text'])), $words);
		$emailParams = array('subject' => $title, 'to' => osc_contact_email(), 'to_name' => osc_page_title(), 'body' => $body, 'alt_body' => $body,);
		osc_sendMail($emailParams);
	}
}
...
function osc_sendMail( array $params )
{
	...
	$email->setFrom( $params['from'], $params['from_name'] );
	$email->setSubject( $params['subject'] );
	$email->addRecipient( $params['to'], $params['to_name'] );
