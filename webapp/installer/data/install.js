    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.status == 200) {
                ...
                alert("Error:<br/>"+data);
                window.location = 'index.php?step=4&error_location=1';
            }
        },
        error: function(data) {
    ...
    if( $("#t_location").length != 0 ) {
        $("#t_location").autocomplete({
            ...
            source: function(text, add){
                ...
                var url = 'http://geo.opensourceclassifieds.org/geo.services.php?callback=?&action=' + type + '&max=5&country=' + country;
                $.jsonp({
                    "url": url,
                    "data": text,
                    "success": function(json) {
...
function change_to_international(input) {
    if(input.checked) {
        ...
        $("#region-box").css('display','none');
        $.jsonp({
            "url": "http://geo.opensourceclassifieds.org/geo.services.php?callback=?&action=country&max=1&term=",
            "error": function(d,msg) {
                no_internet();
            }
        });
